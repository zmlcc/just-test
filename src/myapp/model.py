...
class User(Base):
    ...
    project = db.relationship("Project", secondary=prj_user, lazy="select")
    project_quota = db.Column(db.Integer, default=1)
...
class Project(Base):
    ...
    user = db.relationship("User", secondary=prj_user, lazy='select')
...
class Account(Base):
    ...
    __table_args__ = (db.Index(
        'account_idx', 'cluster_id', 'user_id', unique=True), )
...
class Namespace(Base):
    ...
    __table_args__ = (db.Index(
        'namespace_idx', 'cluster_id', 'project_id', unique=True), )
